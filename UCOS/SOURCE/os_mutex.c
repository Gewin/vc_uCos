...
#if OS_ARG_CHK_EN > 0u
    if (pevent == (OS_EVENT *)0) {                     /* 无效的ECB指针    */
        *perr = OS_ERR_PEVENT_NULL;
        return (OS_FALSE);
    }
#endif
    if (pevent->OSEventType != OS_EVENT_TYPE_MUTEX) {  /* 判断ECB类型是否有效     */
...
#if OS_ARG_CHK_EN > 0u
    if (prio >= OS_LOWEST_PRIO) {                          /* 无效的任务优先级 */
        *perr = OS_ERR_PRIO_INVALID;
        return ((OS_EVENT *)0);
    }
#endif
    if (OSIntNesting > 0u) {                               /* 查看是否是在中断服务程序ISR中调用本函数 */
        *perr = OS_ERR_CREATE_ISR;                         /* 不能在中断服务程序ISR中调用本函数，因此返回错误信息*/
        return ((OS_EVENT *)0);
    }
    OS_ENTER_CRITICAL();
    if (OSTCBPrioTbl[prio] != (OS_TCB *)0) {               /* 优先级不能已经存在*/
        OS_EXIT_CRITICAL();                                /* T优先级已经存在       */
                       ...
                       INT8U     *perr)
{
    ...
    INT8U      pip;                                        /* Priority inheritance priority 优先级继承优先级 */
    INT8U      prio;
    OS_TCB    *ptcb;
#if OS_CRITICAL_METHOD == 3u                               /* Allocate storage for CPU status register */
    OS_CPU_SR  cpu_sr = 0u;
    ...
    if (OSIntNesting > 0u) {                               /* 查看是否是在中断服务程序ISR中调用本函数 */
        *perr = OS_ERR_DEL_ISR;                             /* 不能在中断服务程序ISR中删除*/
        return (pevent);
    }
    OS_ENTER_CRITICAL();
    if (pevent->OSEventGrp != 0u) {                        /* 查看任务是否在等待互斥信号量       */
        tasks_waiting = OS_TRUE;                           /* 是的  */
    } else {
        tasks_waiting = OS_FALSE;                          /* 不是   */
    }
    switch (opt) {
                     ...
                     OS_MUTEX_DATA  *p_mutex_data)
{
    ...
    if (OSIntNesting > 0u) {                               /* 查看是否是在中断服务程序ISR中调用本函数 */
        return (OS_ERR_QUERY_ISR);                         /* 不能在中断服务程序ISR中调用本函数，因此返回错误信息*/
    }
#if OS_ARG_CHK_EN > 0u
    if (pevent == (OS_EVENT *)0) {                         /* 无效的ECB指针*/
        return (OS_ERR_PEVENT_NULL);
